...
if __name__ == "__main__":
    ...
    parser.add_argument("--min-q-weight", type=float, default=1.0, help="the cql alpha")
    parser.add_argument(
    ...
    parser.add_argument("--eval-steps", type=int, default=125000, help="the step num for eval after each epoch")
    ...
    parser.add_argument("--iters", type=int, default=100, help="total iters of experiments")
    
    
    
    # args = parser.parse_args()
    args = parser.parse_args('--seed 1 --env-id BreakoutNoFrameskip-v4 --buffer-num 1'.split(' '))
...
for iteration in range(args.iters):
    ...
    print("iter={}, episodic_return={:.2f}".format(iteration, np.mean(episodic_returns)))
    writer.add_scalar("Charts/episodic_return", np.mean(episodic_returns), iteration) 
    writer.add_scalar("Charts/episodic_return_var", np.var(episodic_returns), iteration)
    writer.add_scalar("Charts/episode_num", len(episodic_returns), iteration)  
    writer.add_scalar("Charts/episode_length", np.mean(episode_lengths), iteration)            
    ...
    for dataset_id in dataset_ids:
        ...
        data_loader = iter(torch.utils.data.DataLoader(dataset, batch_size=args.batch_size, num_workers=8))
    ...
    for train_step in range(args.train_step):
        if train_step % args.train_frequency == 0:
            ...
            if global_train_step % 1000 == 0:
                ...
                writer.add_scalar("losses/min_q_loss", min_q_loss, global_train_step)
                writer.add_scalar("losses/average_q_values", old_val.mean().item(), global_train_step)
                writer.add_scalar("losses/log_exp_sum_q_values", negative_sampling.item(), global_train_step)
                writer.add_scalar("losses/dataset_expected_q_values", dataset_expec.item(), global_train_step)
