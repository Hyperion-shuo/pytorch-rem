...
experiment_name = f"{args.exp_name}__{args.env_id}__{cur_time}__{args.seed}"
...
class ExperienceReplayDataset(IterableDataset):
    def __init__(self, env_id, offline_dataset_id):
        offline_env_id = (
        re.sub(r"(?<!^)(?=[A-Z])", "-", env_id).lower().replace("no-frameskip-v0", "") + offline_dataset_id
        )
...
for iteration in range(args.iters):
    ...
    print("iter={}, mean_episodic_return={:.2f}".format(iteration, np.mean(episode_returns)))
    writer.add_scalar("Charts/episodic_return", np.mean(episode_returns), iteration)  
    ...
    for train_step in range(args.train_step):
        if train_step % args.train_frequency == 0:
            ...
            if global_train_step % 1000 == 0:
                ...
                writer.add_scalar("losses/average_target_max_values", target_max.mean().item(), global_train_step)
                writer.add_scalar("losses/log_sum_exp_q_values", negative_sampling.item(), global_train_step)
    ...
    writer.add_scalar("Charts/train_time", train_time, iteration)         
