...
if __name__ == "__main__":
    ...
    parser.add_argument("--epoches", type=int, default=1000, help="total timesteps of the experiments")
    parser.add_argument(
        "--torch-deterministic",
    ...
    parser.add_argument("--eval-times", type=int, default=5, help="the episodes num for eval after each epoch")
    # parser.add_argument("--train-frequency", type=int, default=4, help="the frequency of training")
    
    
    # args = parser.parse_args()
    args = parser.parse_args('--seed 1 --env-id BreakoutNoFrameskip-v4 --eval-times 15'.split(' '))
    if not args.seed:
...
PATH = "/home/shenshuo/workspace/rem/runs/BreakoutNoFrameskip-v4__offline_dqn__1__1646836076/model.pth"
...
for epoch in range(args.epoches):
    ...
    epsilon = 0.01
    episodic_returns = []
    for eval_episode_num in range(args.eval_times):
        while not done:
            obs = np.array(obs)
    ...
    print("epoch={}, episodic_return={:.2f}".format(epoch, np.mean(episodic_returns)))
    print("epoch=", epoch, "episodic_returns=", episodic_returns)
    writer.add_scalar("charts/episodic_return", np.mean(episodic_returns), epoch)          
