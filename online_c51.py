...
if __name__ == "__main__":
    ...
    parser.add_argument("--learning-start", type=int, default=80000, help="collect samples into replay bufer before tranning")
    ...
    parser.add_argument("--v-min", type=int, default=-10, help="C51 categorical v_min")
    parser.add_argument("--v-max", type=int, default=10, help="C51 categorical v_max")
    parser.add_argument("--n-atoms", type=int, default=51, help="C51 categorical atom num")
      
    args = parser.parse_args()
#     args = parser.parse_args('--seed 1 --env-id PongNoFrameskip-v0 --track --wandb-project-name \
# wandb_test --exp-name online_c51_pong'.split(' '))
    if not args.seed:
...
class QNetwork(nn.Module):
    def __init__(self, env, frames=4, n_atoms=51, v_min=10, v_max=10):
    ...
    def get_action(self, x, action=None):
        ...
        pmfs = torch.softmax(logits.view(len(x), env.action_space.n, self.n_atoms), dim=-1)
        # (batch_size, action_n, n_atoms) * (n_atoms,)
        # element-wise dot, broadcast
        # sum along last axis, reduce to (batch_size, action_n)
        q_values = (pmfs * self.atoms).sum(-1)
        if action is None:
...
# loss defined later, c51 use cross-entropy loss
# loss_fn = nn.MSELoss()
...
for global_step in range(args.total_timesteps):
    ...
    obs = np.array(obs)
    action, pmf, q_values, pmfs = q_network.get_action(torch.Tensor(obs).to(device).reshape((1,) + obs.shape))
    ...
    if global_step > args.learning_start and global_step % args.train_frequency == 0:
        ...
        with torch.no_grad():
            ...
            # (batch_size, n_atom)
            next_atoms = s_rewards.unsqueeze(-1) + args.gamma * q_network.atoms * (1 - s_dones.unsqueeze(-1))
            delta_z = q_network.atoms[1] - q_network.atoms[0]
            # (batch_size, n_atom)
            tz = next_atoms.clamp(args.v_min, args.v_max)

            # (batch_size, n_atom)
            # assgin target categrical to neighbor
            b = (tz - args.v_min) / delta_z
            l = b.floor().clamp(0, args.n_atoms - 1)
            u = b.ceil().clamp(0, args.n_atoms - 1)

            # 如果b是整数，就会出现b=l=u
            # 此时把1.0权重分给l或者u皆可（两者相等）
            m_l = (u + (l==u).float() - b) * next_pmfs
            m_u = (b - l) * next_pmfs
            target_pmfs = torch.zeros_like(next_pmfs)
            for i in range(target_pmfs.size(0)):
                target_pmfs[i].index_add_(0, l[i].long(), m_l[i])
                target_pmfs[i].index_add_(0, u[i].long(), m_u[i])

        _, old_pmfs, _, _ = q_network.get_action(s_obs, s_actions)
        # clamp 一下防止log的数值太大
        loss = (-(target_pmfs * old_pmfs.clamp(min=1e-5).log()).sum(-1)).mean()
        if global_step % 1000 == 0:
            writer.add_scalar("losses/cross_entropy_loss", loss, global_step)
            writer.add_scalar("losses/average_q_values", (old_pmfs * q_network.atoms).sum(-1).mean().item(), global_step)
            writer.add_scalar("losses/average_target_max_values", (next_pmfs * tz).sum(-1).mean().item(), global_step)
